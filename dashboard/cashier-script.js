...
const PAYOS_SETTINGS = {
  ...
  CHECKSUM_KEY:
    "a7e6a0d36be0b35e9977fd5486896b50d2457d71b2f5fb8968a52a6fa4ea5527", // Replace with your Checksum Key
                ...
                <div class="payment-order h-100">
                    ...
                    <div class="payment-content">
                        ...
                        <div class="d-flex justify-content-between gap-2 mt-3">
                            <button class="btn btn-outline-secondary flex-fill" onclick="printPendingInvoice('${
                              order.id
                            }')">
                                <i data-lucide="printer" style="width: 16px; height: 16px;"></i>
                                In hóa đơn
                            </button>
                            <button class="btn btn-enhanced btn-success flex-fill" onclick="openPaymentModal('${
                              order.id
                            }')">
...
function renderRecentPayments() {
  ...
  container.innerHTML = completedOrders
    .map((order) => {
      ...
      const isPayOSPayment =
        order.paymentMethod && order.paymentMethod.includes("PayOS");
            ...
            <div class="recent-payment">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <div>
                        ...
                        <small class="text-muted">${order.table} - ${getTimeAgo(
        order.paymentTime
      )}</small>
                        ${
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <span class="fw-bold text-success">${formatCurrency(
                          finalAmount
                        )}</span>
                    </div>
                </div>
                <div class="d-flex justify-content-between align-items-center">
                    <div class="text-muted small">
                        ...
                        ${
                          order.discount > 0
                            ? `<span class="ms-3">Giảm giá: ${order.discount}%</span>`
                            : ""
                        }
                    </div>
                    <div class="d-flex gap-1">
                        ...
                        <button class="btn btn-outline-primary btn-sm" onclick="printCompletedOrder('${
                          order.id
                        }')">
    ...
    .map((code) => {
      ...
      return `
        ...
        !isEligible ? "disabled" : ""
                 ...
                 onclick="${
                   isEligible ? `selectDiscountCode('${code.code}')` : ""
                 }">
                ...
                ${
                  !isEligible
                    ? '<div class="text-danger small mt-1">Không đủ điều kiện</div>'
                    : ""
                }
            </div>
        `;
...
function processPayment() {
  ...
  const index = pendingPaymentOrders.findIndex(
...
function printInvoice(isCompleted = false) {
  ...
  const hasQRCode =
    isCompleted && orderToPrint.qrCodeData && orderToPrint.qrCodeData.dataUrl;
  const isPayOSPayment =
    isCompleted &&
    orderToPrint.paymentMethod &&
    orderToPrint.paymentMethod.includes("PayOS");
            ...
            ${
              hasQRCode
                ...
                <div style="text-align: center; border-top: 1px dashed #000; padding-top: 15px; margin-bottom: 15px;">
                    ...
                    <img src="${
                      orderToPrint.qrCodeData.dataUrl
                    }" style="width: 120px; height: 120px; margin: 10px auto; display: block;" alt="QR Code thanh toán">
                    ...
                    ${
                      orderToPrint.qrCodeData.isVietQR &&
                      orderToPrint.qrCodeData.accountNumber
                        ? `
...
async function processPayOSPayment() {
  ...
  try {
    ...
    if (
      !PAYOS_SETTINGS.CLIENT_ID ||
      !PAYOS_SETTINGS.API_KEY ||
      !PAYOS_SETTINGS.CHECKSUM_KEY
    ) {
      throw new Error(
        "Thiếu thông tin cấu hình PayOS. Vui lòng kiểm tra CLIENT_ID, API_KEY và CHECKSUM_KEY."
      );
    }
    ...
    if (error.message.includes("fetch")) {
      errorMessage =
        "Không thể kết nối đến PayOS. Vui lòng kiểm tra kết nối mạng.";
...
function completePayOSOrder(payosEvent, amount) {
  ...
  document.getElementById("successMessage").textContent = `Đơn hàng ${
    currentOrder.id
  } đã được thanh toán thành công qua PayOS. Tổng tiền: ${formatCurrency(
    amount
  )}`;
...
function showQRCodePreview(orderId) {
    ...
    if (isVietQR) {
      qrDataDetails = `
                <div class="row g-2">
                    ...
                    <div class="col-6"><strong>Số tiền:</strong> ${formatCurrency(
                      qrData.amount
                    )}</div>
                    ${
                      qrData.accountNumber
                        ? `<div class="col-6"><strong>STK:</strong> ${qrData.accountNumber}</div>`
                        : ""
                    }
                    ${
                      qrData.accountName
                        ? `<div class="col-6"><strong>Tên TK:</strong> ${qrData.accountName}</div>`
                        : ""
                    }
                    ${
                      qrData.bin
                        ? `<div class="col-6"><strong>Ngân hàng:</strong> ${qrData.bin}</div>`
                        : ""
                    }
                    <div class="col-12"><strong>Nội dung:</strong> TT ${
                      order.id
                    } - ${order.table}</div>
                </div>
            `;
  ...
  const qrPreviewHtml = `
        <div class="text-center">
            ...
            <div class="mb-3">
                <img src="${
                  order.qrCodeData.dataUrl
                }" class="img-fluid" style="max-width: 250px;" alt="QR Code">
            </div>
...
function generateInvoiceWithPayOSQR(order) {
  ...
  invoiceHtml +=
    '<div style="display: flex; justify-content: space-between; margin-bottom: 5px;">';
  invoiceHtml +=
    "<span><strong>Hóa đơn:</strong></span><span>" + order.id + "</span>";
  invoiceHtml += "</div>";
  invoiceHtml +=
    '<div style="display: flex; justify-content: space-between; margin-bottom: 5px;">';
  invoiceHtml +=
    "<span><strong>Bàn:</strong></span><span>" + order.table + "</span>";
  invoiceHtml += "</div>";
  invoiceHtml +=
    '<div style="display: flex; justify-content: space-between; margin-bottom: 5px;">';
  invoiceHtml +=
    "<span><strong>Ngày:</strong></span><span>" +
  ...
  invoiceHtml +=
    '<div style="display: flex; justify-content: space-between; margin-bottom: 5px;">';
  invoiceHtml +=
    "<span><strong>Giờ:</strong></span><span>" +
  ...
  invoiceHtml +=
    '<div style="display: flex; justify-content: space-between; margin-bottom: 5px;">';
  invoiceHtml +=
    '<span><strong>Trạng thái:</strong></span><span style="color: #ff6b35; font-weight: bold;">CHƯA THANH TOÁN</span>';
  invoiceHtml += "</div>";
  invoiceHtml += "</div>";
  ...
  invoiceHtml +=
    '<div style="display: flex; justify-content: space-between; margin-bottom: 3px;">';
  invoiceHtml +=
    "<span>Tổng tiền hàng:</span><span>" + formatCurrency(subtotal) + "</span>";
  invoiceHtml += "</div>";
  invoiceHtml +=
    '<div style="display: flex; justify-content: space-between; margin-bottom: 3px;">';
  invoiceHtml +=
  ...
  invoiceHtml +=
    '<div style="display: flex; justify-content: space-between; font-size: 16px; font-weight: bold;">';
  invoiceHtml +=
    "<span>THÀNH TIỀN:</span><span>" + formatCurrency(finalTotal) + "</span>";
  invoiceHtml += "</div>";
  invoiceHtml += "</div>";
  ...
  invoiceHtml +=
    '<div style="text-align: center; border-top: 1px dashed #000; padding-top: 15px; margin-bottom: 15px;">';
  invoiceHtml +=
    '<p style="margin: 0 0 10px 0; font-size: 12px; font-weight: bold;">MÃ QR THANH TOÁN PAYOS</p>';
  ...
  invoiceHtml +=
    "onload=\"console.log('✅ PayOS QR Image loaded successfully')\" ";
  invoiceHtml +=
    "onerror=\"console.log('❌ PayOS QR Image failed to load: ' + this.src); this.style.backgroundColor='#f8f9fa'; this.style.border='2px dashed #ccc'; this.alt='QR Load Error';\">";
  invoiceHtml +=
    '<p style="margin: 10px 0 5px 0; font-size: 10px; color: #666;">Quét mã QR để thanh toán qua VietQR</p>';
  invoiceHtml +=
    '<p style="margin: 0; font-size: 9px; color: #999;">Ngân hàng: KIENLONGBANK | TK: 0969864739</p>';
  invoiceHtml += "</div>";
  ...
  invoiceHtml +=
    '<p style="margin: 10px 0 0 0; font-size: 10px; color: #ff6b35;">* Đây là hóa đơn tạm. Vui lòng thanh toán để hoàn tất.</p>';
  invoiceHtml += "</div>";
  invoiceHtml += "</div>";
...
function addPayOSQRPrintButton(orderCard, order) {
  ...
  const buttonContainer =
    orderCard.querySelector(".d-flex.gap-2") ||
    orderCard.querySelector(".order-actions");
...
function printPendingInvoiceWithPayOSQR(order) {
  ...
  console.log(
    "✅ PayOS Payment Invoice print window opened for order:",
    order.id
  );
}
...
function printPendingInvoiceFromModal() {
  ...
  console.log(
    "🖨️ Printing pending invoice from modal for order:",
    currentOrder.id
  );
