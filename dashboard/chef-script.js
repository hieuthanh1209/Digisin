    ...
    return `
        <div class="col-lg-6 col-xl-6">
            <div class="order-card">
                <div class="order-header ${order.status} d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <span class="status-badge ${order.status}">${statusColor.label}</span>
                    </div>
                </div>
...
function createEmptyState(status) {
    const messages = {
        pending: { title: 'Không có đơn hàng nào', desc: 'Chưa có đơn hàng mới cần xử lý' },
        preparing: { title: 'Không có đơn đang chế biến', desc: 'Chưa có món nào đang được chế biến' },
        ready: { title: 'Không có món nào sẵn sàng', desc: 'Chưa có món nào hoàn thành chờ phục vụ' },
        completed: { title: 'Chưa có đơn hoàn thành', desc: 'Chưa có đơn hàng nào được hoàn thành hôm nay' }
    };
...
function updateOrderStatus(orderId, newStatus) {
    ...
    const statusMessages = {
        pending: { type: 'warning', title: 'Đơn hàng chờ xử lý', message: `${orderId} đã được chuyển về trạng thái chờ xử lý` },
        preparing: { type: 'info', title: 'Bắt đầu chế biến', message: `${orderId} đã bắt đầu được chế biến` },
        ready: { type: 'success', title: 'Món đã sẵn sàng', message: `${orderId} đã hoàn thành và sẵn sàng phục vụ` },
        completed: { type: 'success', title: 'Đã phục vụ', message: `${orderId} đã được phục vụ thành công` }
    };
...
function getToastIcon(type) {
    const icons = {
        success: '<i data-lucide="check-circle-2" class="text-success" style="width: 18px; height: 18px;"></i>',
        warning: '<i data-lucide="alert-triangle" class="text-warning" style="width: 18px; height: 18px;"></i>',
        info: '<i data-lucide="info" class="text-info" style="width: 18px; height: 18px;"></i>',
        danger: '<i data-lucide="x-circle" class="text-danger" style="width: 18px; height: 18px;"></i>'
    };
