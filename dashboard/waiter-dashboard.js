document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTables() {
        tablesGrid.innerHTML = mockTables.map(table => `
            <div class="card">
                <div class="card-header">
                    <h3>Bàn ${table.number}</h3>
                    <span class="text-gray-500">${getStatusText(table.status)}</span>
                </div>
                <div class="card-content">
                    ${table.order ? renderOrderSummary(table.order) : 'Chưa có đơn hàng'}
                </div>
    ...
    function getStatusText(status) {
        switch (status) {
            ...
            case 'ordered': return 'Đã đặt món';
    ...
    function renderMenuItems() {
        menuItemsContainer.innerHTML = mockMenu.map(item => `
    ...
    window.submitOrder = function() {
        ...
        const items = Array.from(document.querySelectorAll('#menuItems input[type="number"]'))
            .map(input => ({
                id: parseInt(input.dataset.itemId),
                quantity: parseInt(input.value),
                name: mockMenu.find(item => item.id === parseInt(input.dataset.itemId)).name
            }))
